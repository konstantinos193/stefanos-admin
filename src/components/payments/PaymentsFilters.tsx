...
const STATUS_OPTIONS = [
  ...
  { value: 'COMPLETED', label: 'Ολοκληρωμένη', color: 'bg-green-500' },
  { value: 'PENDING', label: 'Εκκρεμής', color: 'bg-yellow-500' },
  { value: 'FAILED', label: 'Αποτυχημένη', color: 'bg-red-500' },
  { value: 'REFUNDED', label: 'Επιστροφή', color: 'bg-blue-500' },
  { value: 'PARTIALLY_REFUNDED', label: 'Μερική Επιστροφή', color: 'bg-orange-500' },
]

const METHOD_OPTIONS = [
  { value: '', label: 'Όλες οι Μέθοδοι' },
  { value: 'card', label: 'Κάρτα' },
  { value: 'bank', label: 'Τραπεζική Μεταφορά' },
...
export function PaymentsFilters({ filters, onFiltersChange }: PaymentsFiltersProps) {
  ...
  const handleMethodChange = useCallback((method: string) => {
    onFiltersChange({ ...filters, method: method || undefined, page: 1 })
    setMethodOpen(false)
  }, [filters, onFiltersChange])
  ...
  const selectedMethod = METHOD_OPTIONS.find(m => m.value === (filters.method || ''))
    ...
    <div className="card">
      <div className="flex flex-col xl:flex-row xl:items-center gap-4">
        ...
        <div className="flex-1 relative">
          ...
          <input
            ...
            placeholder="Αναζήτηση πληρωμών, κρατήσεων, επισκεπτών..."
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          ...
          <div ref={statusRef} className="relative">
            <button
              ...
              className="input w-full sm:w-48 text-left flex items-center gap-2 cursor-pointer"
            >
            ...
            {statusOpen && (
              <div className="absolute top-full left-0 mt-2 w-full sm:w-48 bg-[#1e293b] border border-[#334155] rounded-xl shadow-2xl z-50 py-1 animate-slide-down overflow-hidden">
          ...
          <div ref={methodRef} className="relative">
            <button
              ...
              className="input w-full sm:w-40 text-left flex items-center gap-2 cursor-pointer"
            >
              <span className="truncate">{selectedMethod?.label || 'Όλες οι Μέθοδοι'}</span>
              <svg className={`ml-auto h-4 w-4 text-slate-400 shrink-0 transition-transform ${methodOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
              </svg>
            </button>

            {methodOpen && (
              <div className="absolute top-full left-0 mt-2 w-full sm:w-40 bg-[#1e293b] border border-[#334155] rounded-xl shadow-2xl z-50 py-1 animate-slide-down overflow-hidden">
                {METHOD_OPTIONS.map((opt) => (
                  <button
                    key={opt.value}
                    onClick={() => handleMethodChange(opt.value)}
                    className={`w-full px-4 py-2.5 text-left hover:bg-slate-700/50 transition-colors text-sm ${
                      (filters.method || '') === opt.value ? 'bg-slate-700/70 text-white' : 'text-slate-300'
                    }`}
                  >
                    {opt.label}
                  </button>
      ...
      {hasActiveFilters && (
        <div className="flex flex-wrap items-center gap-2 mt-4 pt-4 border-t border-slate-700/50">
          ...
          {filters.method && (
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-slate-700/50 text-slate-300 text-xs rounded-lg">
              Μέθοδος: {METHOD_OPTIONS.find(m => m.value === filters.method)?.label}
              <button onClick={() => onFiltersChange({ ...filters, method: undefined, page: 1 })}>
                <X className="h-3 w-3 text-slate-500 hover:text-slate-300" />
              </button>
            </span>
          )}
