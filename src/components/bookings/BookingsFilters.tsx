...
export function BookingsFilters({ filters, onFiltersChange }: BookingsFiltersProps) {
  ...
  // Sync internal search when filters.search changes externally (header search)
  useEffect(() => {
  ...
  // Preserve status (controlled by tabs), only clear search + dates
  const clearAll = useCallback(() => {
    setSearchValue('')
    onFiltersChange({ limit: filters.limit, status: filters.status })
    ...
    <div className="card">
      <div className="flex flex-col xl:flex-row xl:items-center gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          ...
          <div ref={calendarRef} className="relative">
            ...
            {calendarOpen && (
              <div className="absolute top-full right-0 sm:left-0 mt-2 bg-[#1e293b] border border-[#334155] rounded-xl shadow-2xl z-50 p-4 w-80">
                <div className="flex items-center justify-between mb-4">
                  <button onClick={prevMonth} className="p-1.5 hover:bg-slate-700 rounded-lg transition-colors">
                    <ChevronLeft className="h-4 w-4 text-slate-300" />
                  </button>
                  <span className="text-sm font-semibold text-slate-100">
                    {MONTHS_GR[calendarMonth]} {calendarYear}
                  </span>
                  <button onClick={nextMonth} className="p-1.5 hover:bg-slate-700 rounded-lg transition-colors">
                  ...
                  {calendarDays.map((day, i) => {
                    ...
                    return (
                      <button
                        ...
                        className={`h-9 text-sm font-medium transition-all relative
                          ${start || end
                ...
                <div className="flex items-center justify-between mt-4 pt-3 border-t border-slate-700">
                  <button
                    onClick={() => {
                      ...
                      onFiltersChange({ ...filters, dateFrom: todayStr, dateTo: next30.toISOString().slice(0, 10), page: 1 })
                      setSelectingEnd(false)
                      setCalendarOpen(false)
                    }}
                  ...
                  <button
                    onClick={() => { clearDates(); setCalendarOpen(false) }}
          ...
          {hasActiveFilters && (
            <button
              onClick={clearAll}
              className="flex items-center justify-center gap-2 whitespace-nowrap text-slate-400 hover:text-slate-200 hover:bg-slate-700/50 rounded-xl transition-colors px-4 py-3"
      ...
      {hasActiveFilters && (
        <div className="flex flex-wrap items-center gap-2 mt-4 pt-4 border-t border-slate-700/50">
          ...
          {filters.search && (
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-slate-700/50 text-slate-300 text-sm rounded-lg">
